...
var _load = function(reqs, dump, cb){
  var req = reqs.shift();
  if(!req){
    cb();
    return;
  }
  Name.find(req, function (err, names) {
    if (!err) {
      ...
      if(!dump[req.nametype]){
        dump[req.nametype] = {};
      }
      dump[req.nametype][req.lang] = names;
      _load(reqs, dump, cb);
    } else {
      return res.send(err);
    }
...
exports.dump = function(req, res) {
  var reqs = [];
  var dump = {};
  'family male female'.split(' ').forEach(function(nametype){
    'en de fr it es sv fi ru cs ja'.split(' ').forEach(function(lang){
      reqs.push({
        nametype: nametype,
        lang: lang
      })
